...
interface ProcessRoundPayload {
  gameId?: string;
  round?: number;
}

interface RefundTriggerResult {
  error?: string;
  response?: unknown;
  triggered: boolean;
}
...
async function triggerCancellationRefunds(
  gameId: string
  ...
  const response = await fetch(`${supabaseUrl}/functions/v1/process-refund`, {
    body: JSON.stringify({
      gameId,
      reason:
        "Game cancelled because minimum players were not met before kickoff.",
...
serve(async (request) => {
  ...
  try {
    ...
    const { data: game, error: gameError } = await supabase
      .from("games")
      .select(
        "current_round, id, manager_id, min_players, rebuy_deadline, starting_round, status, wipeout_mode"
      )
      .eq("id", gameId)
      .maybeSingle();
    ...
    const earliestKickoff = roundFixtures[0]?.kickoff_time
      ? new Date(roundFixtures[0].kickoff_time).getTime()
      : null;
    ...
    const roundTeamsSorted = [...uniqueRoundTeams.values()].sort(
      (left, right) => left.name.localeCompare(right.name)
    );

    const { data: alivePlayersResult, error: alivePlayersError } =
      await supabase
        .from("game_players")
        .select("id, user_id")
        .eq("game_id", gameId)
    ...
    if (
      ...
      if (
        ...
        rebuyDeadlineTimestamp > Date.now()
      ) {
        return new Response(
          JSON.stringify({
            assigned: 0,
      ...
      const rebuyerIds = [
        ...new Set((rebuyPayments ?? []).map((payment) => payment.user_id)),
      ];
      ...
      if (rebuyerIds.length >= 2) {
        ...
        const {
          data: refreshedAlivePlayers,
          error: refreshedAlivePlayersError,
        } = await supabase
          .from("game_players")
          .select("id, user_id")
          .eq("game_id", gameId)
        ...
        if (rebuyerIds.length === 1) {
          ...
          rebuyRefundTrigger = await triggerSingleRebuyerRefund(
            gameId,
            targetRound,
            rebuyerIds[0]
          );
        ...
        const { data: lastRoundSurvivors, error: lastRoundSurvivorsError } =
          await supabase
            .from("game_players")
            .select("user_id")
            .eq("game_id", gameId)
        ...
        const winnerIds = [
          ...new Set(
            (lastRoundSurvivors ?? []).map((player) => player.user_id)
          ),
        ];
    ...
    for (const player of alivePlayers) {
      ...
      const usedTeamIds = new Set(
        (usedPicks ?? []).map((pick) => pick.team_id)
      );
      const autoAssignedTeam = roundTeamsSorted.find(
    ...
    const message =
      error instanceof Error ? error.message : "Unknown process-round error";
    const status =
