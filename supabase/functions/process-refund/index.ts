...
function parseScenario(value: string | undefined): RefundScenario {
  if (value === 'game_cancelled' || value === 'kick_player' || value === 'single_rebuyer') {
    return value
  }
...
function toStripeRefundReason(scenario: RefundScenario): Stripe.RefundCreateParams.Reason {
  if (scenario === 'single_rebuyer') {
    return 'duplicate'
  }
...
function getNotificationContent(scenario: RefundScenario, reason: string | null): NotificationContent {
...
serve(async (request) => {
  ...
  try {
    const payload = (await request.json().catch(() => ({}))) as ProcessRefundPayload
    ...
    const message = error instanceof Error ? error.message : 'Unknown process-refund error'
    const isAccessError =
