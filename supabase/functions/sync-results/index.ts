...
function mapStatus(
  apiStatus: string
): "finished" | "live" | "postponed" | "scheduled" {
...
serve(async (request) => {
  ...
  try {
    ...
    const teamByExternalId = new Map(
      (teams as ExistingTeam[] | null)?.map((team) => [
        team.external_id,
        team.id,
      ]) ?? []
    );

    const payload = await footballDataRequest<FootballDataMatchesResponse>(
    ...
    const { data: existingFixtures, error: existingFixturesError } =
      await supabase
        .from("fixtures")
        .select("id, external_id")
        .eq("league_id", leagueId);
