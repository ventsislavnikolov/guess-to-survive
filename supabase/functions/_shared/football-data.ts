...
interface FootballDataRequestOptions {
  method?: "GET";
...
async function getFootballDataApiKey() {
  ...
  const response = await fetch(
    `${supabaseUrl}/rest/v1/rpc/get_runtime_secret`,
    {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Unable to resolve football-data API key from runtime secret: ${body}`
    );
  }
  ...
  if (typeof value !== "string" || !value) {
    throw new Error(
      "Runtime secret lookup returned an empty football-data API key"
    );
  }
