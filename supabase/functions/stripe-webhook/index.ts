...
async function handleCheckoutCompleted(
  ...
  const rebuyRound = parseRebuyRound(
    session.metadata?.rebuy_round,
    paymentType
  );
  ...
  const processingFee = Number.isFinite(metadataProcessingFee)
    ? metadataProcessingFee
    : 0;
    ...
    await sendEmailToUserId(supabase, userId, {
      body:
        paymentType === "rebuy"
          ...
          : "Payment successful. Your spot in the game is confirmed.",
      subject:
        paymentType === "rebuy" ? "Rebuy confirmed" : "Payment confirmed",
      title: paymentType === "rebuy" ? "Rebuy confirmed" : "Payment confirmed",
    });
  } catch (emailError) {
...
async function handleChargeRefunded(charge: Stripe.Charge, eventId: string) {
  ...
  const latestRefund = refunds.reduce<Stripe.Refund | null>(
    (latest, current) => {
      if (!latest) {
        return current;
      }
  ...
  if ((refundedPayments?.length ?? 0) > 0) {
    const notifications = (refundedPayments ?? []).map((payment) => ({
      ...
      title:
        payment.payment_type === "rebuy"
          ? "Rebuy payment refunded"
          : "Payment refunded",
      type:
        payment.payment_type === "rebuy"
          ? "rebuy_payment_refunded"
          : "payment_refunded",
