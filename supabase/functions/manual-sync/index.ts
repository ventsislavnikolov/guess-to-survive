...
function parseTargets(payload: ManualSyncPayload): ManualSyncTarget[] {
  ...
  if (Array.isArray(payload.targets) && payload.targets.length > 0) {
    const filtered = payload.targets.filter((target): target is ManualSyncTarget => {
...
serve(async (request) => {
  ...
  try {
    ...
    const rawPayload = (await request.json().catch(() => ({}))) as ManualSyncPayload
    ...
    const status = message === 'Admin access required' || message === 'Unauthorized' ? 403 : 400
