...
serve(async (request) => {
  if (request.method !== 'POST') {
    return new Response(JSON.stringify({ error: 'Method not allowed' }), { headers: JSON_HEADERS, status: 405 })
  }
  ...
  try {
    ...
    const candidateGames = (games ?? []).filter((game) => typeof game.current_round === 'number' && game.current_round > 0)
    ...
    const message = error instanceof Error ? error.message : 'Unknown send-round-reminders error'
    const status = message === 'Unauthorized' ? 403 : 500
    return new Response(JSON.stringify({ error: message }), { headers: JSON_HEADERS, status })
  }
})
