...
serve(async (request) => {
  ...
  try {
    ...
    const payload = await footballDataRequest<FootballDataTeamsResponse>('/competitions/PL/teams')
    ...
    const existingByExternalId = new Map(
      (existingTeams as ExistingTeam[] | null)?.map((team) => [team.external_id, team.id]) ?? [],
    )
    ...
    if (teamsToInsert.length > 0) {
      const { error: insertTeamsError } = await supabase.from('teams').insert(teamsToInsert)
      if (insertTeamsError) {
