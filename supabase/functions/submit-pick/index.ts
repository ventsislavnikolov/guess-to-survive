...
serve(async (request) => {
  ...
  try {
    ...
    const title =
      action === "created"
        ? `Pick submitted for Round ${round}`
        : `Pick updated for Round ${round}`;
    const body =
      action === "created"
    ...
    return new Response(JSON.stringify({ action, pickId }), {
      headers: CORS_HEADERS,
    });
  } catch (error) {
    const message =
      error instanceof Error ? error.message : "Unknown submit-pick error";
