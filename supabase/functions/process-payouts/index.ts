...
interface ProcessPayoutsPayload {
  gameId?: string;
}
...
serve(async (request) => {
  ...
  try {
    ...
    const payload = (await request
      .json()
      .catch(() => ({}))) as ProcessPayoutsPayload;
    const gameId =
    ...
    const calculatedPrizePool = (successfulPayments ?? []).reduce(
      (sum, payment) => {
        const amount =
          typeof payment.entry_fee === "number"
            ? payment.entry_fee
            : Number(payment.entry_fee ?? 0);
    ...
    const winnerIds = [
      ...new Set((winners ?? []).map((winner) => winner.user_id)),
    ];
    ...
    const connectByUserId = new Map(
      (profiles ?? []).map((profile) => [profile.id, profile.stripe_connect_id])
    );
    ...
    for (const [index, winnerId] of winnerIds.entries()) {
      ...
      const { error: processingUpsertError } = await supabase
        .from("payouts")
        .upsert(
          {
            amount,
      ...
      try {
        ...
        const { error: completedUpsertError } = await supabase
          .from("payouts")
          .upsert(
            {
              amount,
    ...
    const message =
      error instanceof Error ? error.message : "Unknown process-payouts error";
    const status =
