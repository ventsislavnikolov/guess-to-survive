...
interface CreateRebuyCheckoutPayload {
  gameId?: string;
}
...
serve(async (request) => {
  ...
  try {
    ...
    const payload = (await request
      .json()
      .catch(() => ({}))) as CreateRebuyCheckoutPayload;
    const gameId =
    ...
    const { data: game, error: gameError } = await supabase
      .from("games")
      .select(
        "currency, current_round, entry_fee, id, name, rebuy_deadline, status, wipeout_mode"
      )
      .eq("id", gameId)
      .maybeSingle();
    ...
    const { data: existingRebuyPayment, error: existingPaymentError } =
      await supabase
        .from("payments")
        .select("id, status")
        .eq("game_id", gameId)
    ...
    if (
      ...
      ["pending", "refund_pending", "succeeded"].includes(
        existingRebuyPayment.status
      )
    ) {
    ...
    const message =
      error instanceof Error ? error.message : "Unknown rebuy checkout error";
