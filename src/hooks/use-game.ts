...
type PublicGameDetailRow =
  Database["public"]["Functions"]["get_public_game_detail"]["Returns"][number];
...
interface KickPlayerWithRefundInput {
  gameId: string;
  reason: string;
  userId: string;
}
...
export type GameDetail = PublicGameDetailRow & {
  game_players: Pick<
    GamePlayerRow,
    "eliminated_round" | "id" | "is_rebuy" | "joined_at" | "status" | "user_id"
  >[];
};
...
export function useCreateRebuyCheckout() {
  ...
  return useMutation({
    mutationFn: async (gameId: string): Promise<CreateCheckoutResponse> => {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/create-rebuy-checkout`,
        {
          body: JSON.stringify({ gameId }),
          headers: {
            Authorization: `Bearer ${session.access_token}`,
      ...
      if (!response.ok) {
        throw new Error(
          payload?.error ?? "Unable to create rebuy checkout session."
        );
      }
...
export function useKickPlayerWithRefund() {
  ...
  return useMutation({
    mutationFn: async (
      input: KickPlayerWithRefundInput
