...
function ResetPasswordPage() {
  ...
  useEffect(() => {
    ...
    const verifyResetSession = async () => {
      try {
        ...
        setError(
          sessionError instanceof Error ? sessionError.message : 'Could not validate reset link',
        )
      } finally {
        if (active) {
          setVerifying(false)
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(updateError instanceof Error ? updateError.message : 'Could not reset password')
    } finally {
      setLoading(false)
    }
  }
