...
const LOADING_GAME_SKELETON_KEYS = [
  "game-skeleton-1",
  "game-skeleton-2",
  "game-skeleton-3",
  "game-skeleton-4",
...
function GamesPage() {
  ...
  useEffect(() => {
    setPageMeta({
      description:
        "Browse public football survival pools and join the next round.",
  ...
  const pageStart =
    data && data.total > 0 ? (currentPage - 1) * data.pageSize + 1 : 0;
    ...
    <section className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Find public games and join the next survival pool.
          </p>
        </div>
        <div className="flex items-center gap-3">
          {isFetching && !isLoading ? (
            <LoadingSpinner label="Refreshing games..." size="sm" />
          ) : null}
          {user ? (
            <Button asChild>
      ...
      <Card className="border-border bg-card/70">
        <CardHeader>
          ...
          <CardDescription>
            Filter by game type, status, and paid-game entry fee range.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
          <div className="space-y-2">
            ...
            <select
              ...
              id="payment-filter"
              onChange={(event) =>
                updatePaymentFilter(event.target.value as PaymentFilter)
              }
              value={paymentFilter}
            >
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              id="sort-by"
              onChange={(event) =>
                updateSortBy(event.target.value as SortOption)
              }
              value={sortBy}
            >
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              min={MIN_ENTRY_FEE}
              onChange={(event) =>
                updateMinEntryFee(Number(event.target.value))
              }
              step={1}
              type="range"
              value={minEntryFee}
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              min={MIN_ENTRY_FEE}
              onChange={(event) =>
                updateMaxEntryFee(Number(event.target.value))
              }
              step={1}
              type="range"
              value={maxEntryFee}
      ...
      {isLoading ? (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {LOADING_GAME_SKELETON_KEYS.map((key) => (
            <Card className="border-border bg-card/70" key={key}>
      ...
      {!(isLoading || isError) && games.length === 0 ? (
        <Card className="border-border bg-card/70">
          <CardHeader>
            ...
            <CardDescription>
              Create one to start building your player pool.
            </CardDescription>
          </CardHeader>
          {user ? (
            <CardContent>
                ...
                <Card className="border-border bg-card/70 transition hover:border-primary/40 hover:bg-card">
                  ...
                  <CardContent className="space-y-2 text-sm">
                    ...
                    <p>
                      ...
                      {game.current_round ?? game.starting_round}
                    </p>
                    <p>
                      <span className="text-muted-foreground">Players:</span>{" "}
                      {game.player_count ?? 0} joined | Min {game.min_players}
