...
function PickPage() {
  ...
  const filteredTeams = useMemo(() => {
    ...
    return availableTeams.filter((team) => team.name.toLowerCase().includes(normalizedSearch))
  }, [availableTeams, searchTerm])

  const selectedTeam = availableTeams?.find((team) => team.id === selectedTeamId) ?? null
  const usedTeamIds = useMemo(() => new Set((myPicks ?? []).map((pick) => pick.team_id)), [myPicks])
  const roundPick = (myPicks ?? []).find((pick) => pick.round === (round ?? -1)) ?? null
  const roundPickTeam = availableTeams?.find((team) => team.id === roundPick?.team_id) ?? null
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-[0.16em] text-muted-foreground">Make Pick</p>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            ...
            onClick={async () => {
              ...
              if (!window.confirm(`Confirm ${selectedTeam.name} as your pick for round ${roundLabel}?`)) {
                return
              }
              ...
              try {
                ...
                toast.error(pickError instanceof Error ? pickError.message : 'Unable to save your pick.')
              }
            }}
          >
      ...
      {isRoundLocked ? (
        <Card className="border-amber-400/60 bg-amber-500/5">
          <CardContent className="pt-6 text-sm">
            Round lock is active. Picks cannot be submitted or changed after first kickoff.
          </CardContent>
        </Card>
      ) : null}
      ...
      <Card className="border-border bg-card/70">
        <CardHeader>
          ...
          <CardDescription>Review your current round pick and lock deadline before confirming changes.</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-3 text-sm md:grid-cols-3">
          <p>
            <span className="text-muted-foreground">Current pick:</span>{' '}
            {roundPickTeam?.name ?? (roundPick ? `Team #${roundPick.team_id}` : 'No pick yet')}
          </p>
          <p>
            <span className="text-muted-foreground">Deadline:</span>{' '}
            {deadline
              ? new Intl.DateTimeFormat('en-US', { dateStyle: 'medium', timeStyle: 'short' }).format(deadline)
              : 'Unavailable'}
          </p>
          <p>
            <span className="text-muted-foreground">Time left:</span> {formatCountdown(deadline, now)}
          </p>
      ...
      <Card className="border-border bg-card/70">
        <CardHeader className="space-y-3">
          ...
          <CardDescription>
            Select from teams that are playing in the target round. You can switch visual mode.
          </CardDescription>
          <div className="flex flex-wrap items-center gap-2">
            <Input
              className="max-w-sm"
              ...
              {filteredTeams.map((team) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">{formatKickoff(team.fixture_kickoff_time)}</p>
                    {isUsed ? <p className="mt-1 text-xs">Already used in a previous round</p> : null}
                    {isRoundLocked ? <p className="mt-1 text-xs">Round locked</p> : null}
                  </button>
                )
              })}
            </div>
                ...
                return (
                  <li key={team.id}>
                    ...
                    >
                      ...
                      {isSelected ? <Check className="h-4 w-4 text-primary" /> : isUsed ? 'Used' : isRoundLocked ? 'Locked' : null}
                    </button>
                  </li>
                )
              })}
