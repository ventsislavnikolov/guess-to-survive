...
function SpendingHistoryPage() {
  ...
  const grossSpent = paid.reduce((sum, payment) => sum + (payment.total_amount ?? 0), 0)
  const refunded = paid.reduce((sum, payment) => sum + (payment.refunded_amount ?? 0), 0)
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-[0.16em] text-muted-foreground">Responsible gaming</p>
          <h1 className="mt-1 text-2xl font-semibold text-foreground">Spending history</h1>
          <p className="mt-1 text-sm text-muted-foreground">Review paid game entries, refunds, and totals.</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="border-border bg-card/70">
          <CardHeader>
            ...
            <CardDescription>Total charged (including refunded).</CardDescription>
          </CardHeader>
          <CardContent className="text-2xl font-semibold">{formatCurrency(grossSpent, currency)}</CardContent>
        </Card>
        <Card className="border-border bg-card/70">
          <CardHeader>
          ...
          <CardContent className="text-2xl font-semibold">{formatCurrency(refunded, currency)}</CardContent>
        </Card>
        <Card className="border-border bg-card/70">
          <CardHeader>
          ...
          <CardContent className="text-2xl font-semibold">{formatCurrency(netSpent, currency)}</CardContent>
        </Card>
      </div>
      ...
      <Card className="border-border bg-card/70">
        ...
        <CardContent className="space-y-3">
          {isLoading ? <LoadingSpinner label="Loading spending history..." /> : null}
          {!isLoading && isError ? (
            <p className="text-sm text-destructive">{error instanceof Error ? error.message : 'Could not load payments.'}</p>
          ) : null}
          {!isLoading && !isError && paid.length === 0 ? (
            <p className="text-sm text-muted-foreground">No paid entries yet.</p>
          ) : null}
          {!isLoading && !isError
            ? paid.map((payment) => (
                <div key={payment.id} className="space-y-2 rounded-lg border border-border/60 bg-card/70 p-3 text-sm">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="text-right">
                      <p className="font-semibold">{formatCurrency(payment.total_amount ?? 0, payment.currency)}</p>
                      <p className="text-xs text-muted-foreground">{payment.status.replace(/_/g, ' ')}</p>
                    </div>
                  </div>
      ...
      <Card className="border-border bg-card/70">
        <CardHeader>
          ...
          <CardDescription>Enable self-exclusion to block paid games for a set time.</CardDescription>
        </CardHeader>
        <CardContent>
          <Button asChild>
