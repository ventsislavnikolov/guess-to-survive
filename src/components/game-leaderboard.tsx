...
type GameLeaderboardPlayer = Pick<GamePlayerRow, 'eliminated_round' | 'id' | 'joined_at' | 'status' | 'user_id'>
type Identity = Pick<ProfileRow, 'avatar_url' | 'id' | 'username'> | Pick<UserRow, 'avatar_url' | 'id' | 'username'>
type PickWithTeam = Pick<PickRow, 'result' | 'round' | 'team_id' | 'user_id'> & {
  team: Pick<TeamRow, 'id' | 'name' | 'short_name'> | Pick<TeamRow, 'id' | 'name' | 'short_name'>[] | null
}
...
export function GameLeaderboard({
  ...
  const playerIds = useMemo(() => [...new Set(players.map((player) => player.user_id))], [players])
  ...
  const { data, error, isError, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data: picksData, error: picksError } = await supabase
        .from('picks')
        .select('result, round, team_id, user_id, team:teams(id, name, short_name)')
        .eq('game_id', gameId)
  ...
  const rows = useMemo<LeaderboardRow[]>(() => {
    const picksByUser = data?.picksByUser ?? new Map<string, LeaderboardPick[]>()
    const identitiesById = data?.identitiesById ?? new Map<string, Identity>()

    return [...players]
      .map((player) => {
        const identity = identitiesById.get(player.user_id)
        const roundsSurvived = Math.max(0, (player.eliminated_round ?? currentRound) - startingRound)
        const picks = picksByUser.get(player.user_id) ?? []

        return {
          ...player,
          avatar_url: identity?.avatar_url ?? null,
          display_name: getPlayerName(identity?.username ?? null, player.user_id),
          picks,
  ...
  if (players.length === 0) {
    return <p className="text-sm text-muted-foreground">No players have joined this game yet.</p>
  }
      ...
      <table className="min-w-full divide-y divide-border text-left text-sm">
        <thead className="bg-muted/30">
          <tr>
            ...
            <th className="px-3 py-2 font-medium text-muted-foreground">Player</th>
            <th className="px-3 py-2 font-medium text-muted-foreground">Status</th>
            <th className="px-3 py-2 font-medium text-muted-foreground">Rounds Survived</th>
            <th className="px-3 py-2 font-medium text-muted-foreground">Current Pick</th>
            <th className="px-3 py-2 font-medium text-muted-foreground">Teams Used</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-border">
          {rows.map((row, index) => {
            const currentPick = row.picks.find((pick) => pick.round === currentRound) ?? null
            ...
            const hasHiddenCurrentPick = currentPick !== null && !canSeeCurrentPick
            const name = viewerUserId === row.user_id ? 'You' : row.display_name
            ...
            return (
              <tr key={row.id} className="align-top">
                ...
                <td className="px-3 py-3 text-foreground">{row.rounds_survived}</td>
                <td className="px-3 py-3">
                  {currentPick === null ? (
                    <span className="text-muted-foreground">-</span>
                  ) : canSeeCurrentPick ? (
                    <span className="font-medium text-foreground">{formatTeam(currentPick)}</span>
                  ) : (
