  ...
  return (
    <Card className="border-border bg-card text-card-foreground">
      ...
      <CardContent className="space-y-3">
        ...
        {!isLoading && isError ? (
          <p className="text-sm text-destructive">{error instanceof Error ? error.message : 'Could not load stats.'}</p>
        ) : null}
        {!isLoading && !isError && stats ? (
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <Stat label="Total winnings" value={formatCurrency(stats.totalWinnings, currency)} />
